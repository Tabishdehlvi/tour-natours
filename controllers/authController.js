...
const get_email_template_for_reset_password = require('../utils/emailtemplates');
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      new AppError('There is no user exist with that email address.', 404)
    );
  }
  ...
  try {
    await sendEmail({
      ...
      message,
      html: get_email_template_for_reset_password(user.name, resetURL),
    });
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  // 3) Update changed_password_at property for the user
